...
app.post('/api/room', (req, res) => {
  try {
    ...
    db.prepare('INSERT INTO rooms (id, mode, puzzle_seed, created_at) VALUES (?, ?, ?, ?)').run(id, mode, puzzle_seed, created_at);
    const origin = req.get('origin') || req.protocol + '://' + req.get('host');
    const joinLink = `${origin}?room=${id}`;
    res.json({ roomId: id, joinLink, seed: puzzle_seed, mode });
  } catch (e) {
...
app.get('/api/room/:id', (req, res) => {
  const row = db.prepare('SELECT id, mode, puzzle_seed FROM rooms WHERE id = ?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'Room not found' });
  const results = db.prepare(
    'SELECT player_name, score, time_seconds, won, created_at FROM room_results WHERE room_id = ? ORDER BY score DESC, time_seconds ASC'
  ).all(row.id);
  res.json({ roomId: row.id, mode: row.mode, seed: row.puzzle_seed, results });
});
...
app.post('/api/room/:id/result', (req, res) => {
  ...
  const { playerName, score, timeSeconds, won } = req.body;
  const name = (playerName && String(playerName).trim()) || 'Anonymous';
  const created_at = Date.now();
  db.prepare(
    'INSERT INTO room_results (room_id, player_name, score, time_seconds, won, created_at) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(req.params.id, name.substring(0, 50), Number(score) || 0, Number(timeSeconds) || 0, won ? 1 : 0, created_at);
  res.json({ ok: true });
});
...
app.get('/api/room/:id/leaderboard', (req, res) => {
  ...
  if (!room) return res.status(404).json({ error: 'Room not found' });
  const results = db.prepare(
    'SELECT player_name AS playerName, score, time_seconds AS timeSeconds, won, created_at AS createdAt FROM room_results WHERE room_id = ? ORDER BY score DESC, time_seconds ASC'
  ).all(req.params.id);
  res.json(results);
});
